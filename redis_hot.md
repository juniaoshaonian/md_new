...
### 业务数据隔离
**核心思路**：
将某一个热门业务单独分隔出来，不让他影响其他业务。举个例子来说，如果 User 和 Order 共用一个 Redis Cluster，然后 User 导致某个节点成为热点了，那么可以将 User 改为使用另外一个 Redis Cluster；   
...
### 缓存多份
第二个方案就是缓存多份，比如 test_key 是一个热点，那么可以将同样的内容写入到 test_key0，test_key1..., 而后业务在查询的时候，使用轮训或者随机访问其中一个key。这种方式适用于读操作较多的场景，但是会加剧数据的不一致性。
...
### 背景说明
这个面试方案一个是大数据、高性能、高并发榜单的解决方案。结合了上面本地缓存和拆分 key 的方法。不仅可以在谈及 Redis 热点的时候引用这个方案，也可以作为你项目的一个难点。彰显你对大数据，高并发、高性能的独到理解。
...
- 从理论上来说，这 10 个 key 应该在经过 CRC16 计算和槽映射之后，应该均匀分散在 Redis Cluster 不同节点上；
...
这个方案从性能角度来说，最主要的就是直接命中了本地缓存。速度飞快

从高可用的角度来说，即便整个 Redis 集群全崩了，也可以靠着本地缓存撑住。而且这个方案的落地难度也不大，也可以在你的工作中应用。
...
# 大key问题
这里提一下一个和热点key问题很像的一个问题大key问题

所谓的大 key，不是指 key 本身很长，而是指值很大，比如说一个巨大的 JSON 串，一个巨大的 zset，list 等。大 key 一般会引起数据倾斜、QPS 倾斜等问题。或者说大 key 有极大的可能是热点 key，并发极高。

大key的解决方案和热点问题差不多，就是分而治之，你可以参照上面的拆分key的方案来回答。

## 使用hash结构来优化
这里也有一些例外，比如说存储大 JSON 串的时候，可以考虑用哈希结构来优化。

在实践中，很多时候我们都要在 Redis 里面存储一些 JSON 数据，而后带来的缺陷就是每次读写都必须是操作整个 JSON 串。举一个最简单的例子，我们经常会在将 User 转化为一个 JSON 串，而后放入 Redis 中。但是在使用的时候，可能只需要 User 中的部分字段，比如说只需要 nickname 这个字段，但是你依旧不得不整个 JSON 取出来。

那么这种整体读写 JSON 的操作是非常耗时的。所以一种优化方式就是用哈希结构（map）存储。

例如 以前存储的是这个User json对象
```json
{
  "username": "usr1",
  "nickname": "usr1"
...
HSET user:1001 username "usr1" nickname "usr1"

然后拿数据的时候只需要拿对应字段的数据即可。


在实践中，JSON串越大，优化的效果越好，甚至有可能会取得数量级的优化效果。
